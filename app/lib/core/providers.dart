...
final apiClientProvider = Provider<ApiClient>((ref) {
  ...
  client.onSessionExpired =
      () async => ref.read(authControllerProvider.notifier).signOut();
  return client;
});
...
final mediaRepositoryProvider =
    Provider((ref) => MediaRepository(ref.watch(apiClientProvider)));
final nutritionRepositoryProvider =
    Provider((ref) => NutritionRepository(ref.watch(apiClientProvider)));
final trainingRepositoryProvider =
    Provider((ref) => TrainingRepository(ref.watch(apiClientProvider)));
final sleepRepositoryProvider =
    Provider((ref) => SleepRepository(ref.watch(apiClientProvider)));
final coachRepositoryProvider =
    Provider((ref) => CoachRepository(ref.watch(apiClientProvider)));
final momentsRepositoryProvider =
    Provider((ref) => MomentsRepository(ref.watch(apiClientProvider)));
...
final nutritionRangeProvider =
    FutureProvider.family<List<DailyNutrition>, DateRange>((ref, range) => ref
        .watch(nutritionRepositoryProvider)
        .range(range.fromIso, range.toIso));

final bodyMetricsRangeProvider =
    FutureProvider.family<List<BodyMetric>, DateRange>((ref, range) => ref
...
final momentEventsProvider = StreamProvider<SseEvent>((ref) async* {
  ...
  while (alive) {
    final uri = await api.streamUri('/v1/moments/stream', query: {'since': '$since'});
    if (uri == null) {
...
class ConversationsState {
  const ConversationsState(
      {this.items = const [], this.loading = true, this.error});
...
class ConversationsController extends StateNotifier<ConversationsState> {
  ...
  void received(DirectMessage message) {
    ...
    final existing =
        state.items.where((c) => c.userId == other).firstOrNull;
...
final conversationsProvider =
  ...
  final controller =
      ConversationsController(ref.watch(momentsRepositoryProvider));
