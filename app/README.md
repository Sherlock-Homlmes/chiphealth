...
| Snore / sleep-talk detection | Implemented: YAMNet (AudioSet, 521 classes) runs on-device via `tflite_flutter` on every 0.975 s window above the silence floor; snore/talk/cough events coalesce, get 15 s WAV clips (budget: 12 snore clips/night, talk/cough always) and upload through the existing `sleep_audio_clip` media kind. Two independent vetoes keep the phone's own night audio out of the events: YAMNet's media classes (music, TV, white noise) and, because a podcast or an audiobook is speech and scores as none of those, the OS itself — `AudioManager.isMusicActive` / `AVAudioSession.isOtherAudioPlaying` via `audio_session`, polled every 5 s. While the phone is playing, sleep_talk and cough are dropped and the minute is booked as media (so the hypnogram does not call it awake); snore still counts. Web keeps the fallback path (no `dart:ffi`). Model: `assets/models/yamnet.tflite`, sha256 `10c95ea3…317de`. Remaining: thresholds need tuning on real beds. Overnight survival behind a locked screen: Android holds the night open with a `microphone` foreground service (flutter_foreground_task, Kotlin 2.2.20 required); iOS uses the plist's `UIBackgroundModes: audio` plus `AudioInterruptionMode.pauseResume` + `mixWithOthers` on the record session. Neither platform survives the user swiping the app away (accepted). |
| Audio playback | Wired: the per-event play button streams the clip's bytes (data URI on web, `StreamAudioSource` on devices); `MediaRepository.bytes` accepts WAV alongside image signatures. |
